...
source("02a_descriptive_statistics_ILE.R")
...
exp(coef(dag.glm))
((1.2038-1.5266)/1.5266) * 100

summary(dag.glm)
exp(confint(dag.glm))


# Primary model - full sample
primary.glm <- glm(resistant.f ~ ever_deten.f + sexM.f + rural.f + 
                     homeless.f + jobcat.f + edu.f + age.cat.f,
                   data = moldova,
                   family = binomial(link = "logit"))

outs(primary.glm)

# Sensitivity analysis - adjusted for HIV, complete cases only
sensitivity.glm <- dag.glm
...
dag.glm.noH <- glm(resistant.f ~ ever_deten.f + sexM.f + rural.f + homeless.f + jobcat.f + edu.f + age.cat.f, 
               data = moldova,
               family = binomial(link = "logit"))

dag.glm.1 <- glm(resistant.f ~ ever_deten.f*hiv.f+ sexM.f + rural.f + homeless.f + jobcat.f + edu.f + age.cat.f, 
                 data = moldova,
                 family = binomial(link = "logit"))


#imputation 
moldova_impute_raw <- mice(moldova, m=6, method='pmm')
fit <- with(moldova_impute_raw, glm(resistant.f ~ ever_deten.f + 
                                      sexM.f + 
                                      rural.f + 
                                      homeless.f + 
                                      jobcat.f + 
...
pooled <- pool(fit)
moldova_impute_raw$loggedEvents
summary(pooled)
